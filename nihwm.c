...
static void incnborder(const Arg *arg);
...
incnborder(const Arg *arg)
{
	...
	if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
		for (i = 0; i < num; i++) {
			// TODO resize the current border		
		}
	}
}
...
void
killclientsel(const Arg *arg)
{
	Arg temp;

	temp.v = selmon->sel;

	killclient(&temp, arg->i);	
}

// forced = 0 -> normal, forced = 1 -> using kill signal
void
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
		...
		fprintf(stderr, "nihwm: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "nihwm: fatal error: request code=%d, error code=%d\n",
